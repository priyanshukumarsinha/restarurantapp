    ...
    <section className='grid grid-cols-1 md:grid-cols-2 gap-2 w-full ' id='home'>
      <div className='py-2 flex-1 flex flex-col items-start gap-6 justify-start md:items-start' >
        ...
        <p className='text-[2.5rem] lg:text-[4rem] md:text-[3rem] font-bold tracking-wide text-headingColor '>
          The Fastest Delivery in <span className='text-orange-600 text-[3rem] lg:text-[5rem] md:text-[3.5rem] '> Your City</span>
        </p>
      ...
      <div className='py-2 flex-1 relative' >
        <img src={HeroBg} className = "lg:h-650 h-420 w-full lg:w-auto ml-auto" alt="herobg" />
        <div className="w-full h-full absolute top-0 left-0 flex items-center justify-center py-4 xl:px-32 gap-4 flex-wrap drop-shadow-lg">
           {
            heroData && (
            heroData.map((data) => (
                <div key= {data.id} className="lg:w-190  p-4 bg-cardOverlay backdrop-blur-md rounded-3xl flex items-center justify-center flex-col">
                    <img src={data.imgSrc} alt="i1" className='-mt-10 lg:-mt-20 w-20 lg:w-40'/>
